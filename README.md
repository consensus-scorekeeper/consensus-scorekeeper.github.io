...
Open **📱 Buzzers** in the scoreboard and click **Create room** — you get a 4-letter room code and a player link to share. Everyone who opens the link — phone or computer — gets a full-screen buzz button plus the live scoreboard and a browsable list of past questions (with answers, once the moderator has moved on). On a keyboard, **Space** buzzes. A small toggle in the top bar picks whether your buzz fires the instant you press or when you release. First buzz wins — with latency equalization: the server measures each player's ping and adjudicates near-simultaneous buzzes by estimated press time, so being far from the moderator doesn't decide races. The moderator sees who buzzed, reads the highlighted player, and presses **Space** to award the points (or **Esc** to dismiss and re-open the buzzers — normal scoring keys keep working too). If someone joins under a name that isn't on a roster, either click an existing player's row to link them, or add them to a team as a new player — buttons for both appear in the buzz banner and in the Buzzers panel's joiner list. You can even start a game with empty rosters and build the teams as people join with the code.
